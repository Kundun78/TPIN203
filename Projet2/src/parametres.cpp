...
//_ ______________________________________________________________________________________________ _
void 
mise_a_jour(const parametres& params, int width, int height, const char* galaxie_previous, char* galaxie_next,int nbp, int rank)
{
    int i, j;
    ...
    int subheight = height/(nbp-1);
    int remainder = height%(nbp-1);
    int n = rank*subheight+remainder*(rank==(nbp-1));
    int k = (rank-1)*subheight;
    for ( i = k; i < n; ++i )
      {
        for ( j = 0; j < width; ++j )
        {
